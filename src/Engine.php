...
//            'type' => get_class($builder->model),
        ...
        if (!empty($builder->query)) {
            $query = ['query_string' => ['query' => "*{$builder->query}*"]];
...
var_dump($params);exit;
    ...
    public function map(Builder $builder, $results, $model)
    {
        ...
        if ($total === 0 || empty($hits)) {
            return $model->newCollection();
        }

        return $this->formatItems($hits, $model);

//        $keys = collect($hits)->pluck('_id')->values()->all();
//
//        $modelIdPositions = array_flip($keys);
//
//        var_dump($modelIdPositions);exit;
//
...
//        })->values();
    ...
    protected function formatItems($items, $model = null)
    {
        var_dump($items);exit;
        $dictionary = [];
        $key = '_id';
        ...
        if (!is_null($model)) {
            $dictionary = $model->newCollection($dictionary);
        }
